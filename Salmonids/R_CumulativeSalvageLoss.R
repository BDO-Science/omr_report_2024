...
base_df_WR<- data.frame(Date=seq(as.Date('2022-10-01'), as.Date('2023-07-01'), by = 'days'),
                        WR_Cumulative_50=292, WR_Cumulative_75=438,WR_Cumulative_90=525.6,WR_Cumulative_100=584
)
...
write.csv(salvage_data %>% rename(SampleTime = 'Sample Time') %>% filter(as.Date(SampleTime)>="2022-10-01"),file="WY2023_ChinookSalmonSalvageData.csv", row.names = F)
...
wr_salvage_data<-salvage_data %>% mutate(Date=as.Date(SampleTime)) %>% filter(Date>="2022-10-01") %>% filter(LAD_Race=="Winter") %>% group_by(Date) %>%
...
#Print figure
tiff(filename=file.path("output","Figure_WY_Cumulative_loss_Winter-run.tiff"),width=10,height = 6, units = "in",  res=300, compression ="lzw")

par(mar = c(8.1, 4.1, 4.1, 4.1))

plot(wr_salvage_data_cumulative$Date, wr_salvage_data_cumulative$CumulativeLoss, ylim = c(0, 1500), xaxt = "n",col = "green", type = "l", lty = 1, lwd= 2,main="Natural Winter-Run", xlab = "", ylab = "Loss (n)")
axis(1, at=at, format(at, "%b %d %Y"), cex.axis = .7)
lines(wr_salvage_data_cumulative$Date, wr_salvage_data_cumulative$WR_Cumulative_100, col = "black", type = "l", lty = 1)
lines(wr_salvage_data_cumulative$Date, wr_salvage_data_cumulative$WR_Cumulative_90, col = "grey4", type = "l", lty = 2)
lines(wr_salvage_data_cumulative$Date, wr_salvage_data_cumulative$WR_Cumulative_75, col = "grey2", type = "l", lty = 3)
lines(wr_salvage_data_cumulative$Date, wr_salvage_data_cumulative$WR_Cumulative_50, col = "grey1", type = "l", lty = 4)

legend("bottomleft", legend = c("Cumulative Loss", "50% Annual Loss Threshold", "75% Annual Loss Threshold","90% Annual Loss Threshold","100% Annual Loss Threshold"), inset=c(0,-0.40),
       col=c("green","black","black","black","black"), lty=c(1,4,3,2,1), lwd=c(2,1,1,1,1),bty = "n", xpd=TRUE, mar(c(7,7,11,7)), cex = 0.8)
...
base_df_WR<- data.frame(Date=seq(as.Date('2020-02-19'), as.Date('2023-07-01'), by = 'days'), CumBO_Loss=8738
)
...
#Cumulative Loss
wr_salvage_data<-salvage_data %>% mutate(Date=as.Date(SampleTime)) %>% filter(Date>="2020-02-19") %>% filter(LAD_Race=="Winter") %>% group_by(Date) %>%
  summarise(Loss=sum(Loss))
...
#Print figure
tiff(filename=file.path("output","Figure_10yr_Cumulative_loss_Winter-run_threshold.tiff"),width=8,height = 6, units = "in",  res=300, compression ="lzw")
...
# Write csv for the OMR report appendix
write.csv(sth_salvage_data %>% rename(SampleTime = 'Sample Time') %>% filter(as.Date(SampleTime)>="2022-10-01"),file=file.path("output","WY2023_SteelheadSalvageData.csv"), row.names = F)
...
#Cumulative Loss
wy_salvage_data<-sth_salvage_data %>% mutate(Date=as.Date(SampleTime)) %>% filter(Date>="2022-10-01") %>% group_by(Date) %>%
  summarise(Loss=sum(Loss))
...
base_df_STH_Dec_March<- data.frame(Date=seq(as.Date('2022-10-01'), as.Date('2023-03-31'), by = 'days'),
                                   STH_Annual_50=1414*0.5, STH_Annual_75=1414*0.75,STH_Annual_90=1414*0.9,STH_Annual_100=1414
)

base_df_STH_Apr_Jun<- data.frame(Date=seq(as.Date('2023-04-01'), as.Date('2023-06-15'), by = 'days'),
                                 STH_Annual_50=1552*0.5, STH_Annual_75=1552*0.75,STH_Annual_90=1552*0.9,STH_Annual_100=1552
)
...
#Print figure
tiff(filename=file.path("output","Figure_WY_Cumulative_loss_Steelhead.tiff"),width=10,height = 6, units = "in",  res=300, compression ="lzw")

par(mar = c(8.1, 4.1, 4.1, 3.1))

plot(sth_salvage_data_cumulative$Date, sth_salvage_data_cumulative$CumulativeLoss, ylim = c(0, 1600), xaxt = "n",col = "green", type = "l", lty = 1, lwd= 2,main="Natural Steelhead", xlab = "", ylab = "Loss (n)")
axis(1, at=at, format(at, "%b %d %Y"), cex.axis = .7)
lines(sth_salvage_data_cumulative$Date, sth_salvage_data_cumulative$STH_Annual_100, col = "black", type = "l", lty = 1)
lines(sth_salvage_data_cumulative$Date, sth_salvage_data_cumulative$STH_Annual_90, col = "grey4", type = "l", lty = 2)
lines(sth_salvage_data_cumulative$Date, sth_salvage_data_cumulative$STH_Annual_75, col = "grey2", type = "l", lty = 3)
lines(sth_salvage_data_cumulative$Date, sth_salvage_data_cumulative$STH_Annual_50, col = "grey1", type = "l", lty = 4)
abline(v=as.Date('2023-03-31'), col="red",lwd=3)

legend("bottomleft", legend = c("Annual Cumulative Loss", "50% Annual Loss Threshold", "75% Annual Loss Threshold","90% Annual Loss Threshold","100% Annual Loss Threshold"), inset=c(0,-0.40),
       col=c("green","black","black","black","black"), lty=c(1,4,3,2,1), lwd=c(2,1,1,1,1),bty = "n", xpd=TRUE, mar(c(7,7,11,7)), cex = 0.8)
...
#Dec_march
base_df_STH_Dec_March<- data.frame(Date=seq(as.Date('2020-02-19'), as.Date('2023-07-01'), by = 'days')
)
...
#Cumulative Loss
sth_dec_mar<-sth_salvage_data %>% mutate(Date=as.Date(SampleTime)) %>% filter(Date>="2020-02-19") %>% group_by(Date) %>%
...
###############
#April-June
base_df_STH_Apr_Jun<- data.frame(Date=seq(as.Date('2020-02-19'), as.Date('2023-07-01'), by = 'days')
)
...
#Cumulative Loss
sth_apr_jun<-sth_salvage_data %>% mutate(Date=as.Date(SampleTime)) %>% filter(Date>="2020-02-19") %>% group_by(Date) %>%
...
#Print Figure
tiff(filename=file.path("output","Figure_10yr_Cumulative_loss_Steelhead_overPA.tiff"),width=10,height = 6, units = "in",  res=300, compression ="lzw")
